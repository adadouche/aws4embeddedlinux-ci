...
function addNagSuppressions(stack: cdk.Stack) {
  ...
  NagSuppressions.addResourceSuppressionsByPath(stack, `/${stack.stackName}/EmbeddedLinuxCodePipelineBucketDeploymentRole/DefaultPolicy/Resource`,
    [{ id: 'AwsSolutions-IAM5', reason: 'Because these are the default permissions assigned to a CDK default created role.', },]
  );
  NagSuppressions.addResourceSuppressionsByPath(stack, `/${stack.stackName}/EmbeddedLinuxCodePipeline/Artifact/Artifact/CodePipelineActionRole/DefaultPolicy/Resource`,
    [{ id: 'AwsSolutions-IAM5', reason: 'Because these are the default permissions assigned to a CDK default created role.', },]
  );
}
...
describe('EmbeddedLinuxCodePipelineStack cdk-nag AwsSolutions Pack', () => {
  ...
  beforeAll(() => {
    ...
    const outputBucketVMImportBucket = new VMImportBucket(baseStack, 'OutputtBucketVMImportBucket', {
